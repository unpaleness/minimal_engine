...
namespace
{
    ...
    VkPresentModeKHR ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
    {
        ...
        {
            // The best present mode in terms of quality, but consumes more energy
    ...
    void TransitionImageLayout(VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue, VkImage image,
        [[maybe_unused]] VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
    {
        VkCommandBuffer commandBuffer = BeginSingleTimeCommands(device, commandPool);
    ...
    void GenerateMipmaps(VkPhysicalDevice physicalDevice, VkDevice device, VkCommandPool commandPool,
        VkQueue graphicsQueue, VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight,
        uint32_t mipLevels)
    {
    ...
    VkSampleCountFlagBits GetMaxUsableSampleCount(VkPhysicalDevice physicalDevice)
    {
        ...
        for (uint64_t mask = VK_SAMPLE_COUNT_64_BIT; mask > VK_SAMPLE_COUNT_1_BIT; mask >>= 1)
        {
            if (counts & mask)
            {
                return static_cast<VkSampleCountFlagBits>(mask);
            }
        }
...
Render::Render(GLFWwindow* aWindow) :
    window(aWindow)
{
    CreateInstance();
    SetupDebugMessenger();
...
void Render::DrawFrame()
{
    ...
    vkWaitForFences(device, 1, &vkInFlightFence, VK_TRUE, UINT64_MAX);
    ...
    vkResetFences(device, 1, &vkInFlightFence);
    ...
    if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, vkInFlightFence) != VK_SUCCESS)
    {
...
void Render::OnFrameBufferResized()
{
...
void Render::PickPhysicalDevice()
{
    ...
    vkEnumeratePhysicalDevices(instance, &deviceCount, enumeratedDevices.data());

    for (const auto& enumeratedDevice : enumeratedDevices)
    {
        if (IsDeviceSuitable(enumeratedDevice, surface))
        {
            physicalDevice = enumeratedDevice;
...
void Render::CreateTextureImage()
{
    ...
    CreateImage(physicalDevice, device, texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB,
    ...
    CopyBufferToImage(device, commandPool, graphicsQueue, stagingBuffer, textureImage, static_cast<uint32_t>(texWidth),
        static_cast<uint32_t>(texHeight));

    GenerateMipmaps(physicalDevice, device, commandPool, graphicsQueue, textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth,
        texHeight, mipLevels);
...
void Render::CreateColorResources()
{
    ...
    CreateImage(physicalDevice, device, swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorFormat,
...
void Render::CreateDepthResources()
{
    ...
    CreateImage(physicalDevice, device, swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, depthFormat,
        VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
        depthImage, depthImageMemory);

    depthImageView = CreateImageView(device, depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);

    TransitionImageLayout(device, commandPool, graphicsQueue, depthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED,
...
void Render::UpdateUniformBuffer(uint32_t currentImage)
{
    ...
    const glm::mat4 rotateHorizontally =
        glm::rotate(glm::mat4(1.0f), std::sin(time) * glm::radians(15.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    ubo.model =
        glm::rotate(rotateHorizontally, std::cos(time * 0.5f) * glm::radians(5.0f), glm::vec3(0.7071f, 0.7071f, 0.0f));
    ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    ubo.proj = glm::perspective(
        glm::radians(45.0f), swapChainExtent.width / static_cast<float>(swapChainExtent.height), 0.1f, 10.0f);
    // in OpenGL Y coordinate is inverted
    ubo.proj[1][1] *= -1;
